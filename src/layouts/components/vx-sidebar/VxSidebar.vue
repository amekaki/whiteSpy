            ...
            <div @mouseenter="sidebarMouseEntered" @mouseleave="sidebarMouseLeave">
                <div class="header-sidebar flex items-end justify-between" slot="header">
                    <div class="logo flex items-center">
                        <img src="@/assets/images/logo/logo.png" alt="logo" class="w-10 mr-4" v-if="logo">
                ...
                <VuePerfectScrollbar ref="mainSidebarPs" class="scroll-area--main-sidebar pt-2" :settings="settings" @ps-scroll-y="psSectionScroll">
                    <template v-for="(sidebarItem, index) in sidebarItems">
                        ...
                        <template v-else-if="!sidebarItem.header">
                            ...
                            <!-- IF IT'S SINGLE ITEM -->
                            <vx-sidebar-item :currentPath="currentPath" :MaterialIcon="sidebarItem.MaterialIcon" ref="sidebarLink" :key="`sidebarItem-${index}`" v-if="!sidebarItem.submenu" :index="index" :to="sidebarItem.url" :href="sidebarItem.slug == 'external' ? sidebarItem.url : ''" :icon="sidebarItem.icon" :target="sidebarItem.target" :isDisabled="sidebarItem.isDisabled">
                            ...
                            <template v-else>
                                <vx-sidebar-group :currentPath="currentPath" ref="sidebarGrp" :key="`group-${index}`" :openHover="openGroupHover" :group="sidebarItem" :groupIndex="index" open="true"></vx-sidebar-group>
                            </template>
                        </template>
                    </template>
                </VuePerfectScrollbar>
