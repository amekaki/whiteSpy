    ...
    <ul ref="items" :style="styleItems" class="vs-sidebar-group-items">
      <li v-for="(groupItem, index) in group.submenu" :key="index">
        <vx-sidebar-group :group="groupItem" :groupIndex="Number(`${groupIndex}.${index}`)"
                          :open="isGroupActive(groupItem)" :openHover="openHover" :currentPath="currentPath" v-if="groupItem.submenu"/>
        <vx-sidebar-item :index="groupIndex + '.' + index" :to="groupItem.urlList"
                         :currentPath="currentPath" :icon="itemIcon(groupIndex + '.' + index)" :MaterialIcon="groupItem.MaterialIcon" icon-small  :target="groupItem.target" v-else>
