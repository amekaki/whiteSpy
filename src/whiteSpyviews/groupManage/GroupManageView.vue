                        ...
                        <div class="email__actions--single flex items-baseline">
                            ...
                            <vs-dropdown vs-custom-content vs-trigger-click class="cursor-pointer">
                                ...
                                <vs-dropdown-menu style="z-index: 40001">
                                        <vs-dropdown-item v-for="(label, index) in userTags" :key="index">
...
export default{
    ...
    computed: {
        currentMail() {
            return this.$store.getters['email/getMail'](this.userId)
        },
        labelColor() {
            return (label) => {
        ...
        currentMailLabels: {
            get() {
                return this.$store.getters['email/getMail'](this.userId).labels;
            },
            set(value) {
                if(Array.isArray(value)){
                    const payload = { mailId: this.userId, value: value }
    ...
    methods: {
      getUser(){
        ...
        // toggleIsStarred() {
        //     const payload = {mailId: this.userId, value : !this.currentMail.isStarred}
    ...
    // updated() {
    //     if(this.currentMail.unread && this.isSidebarActive) this.$store.dispatch('email/updateMailUnread', {mails: [this.userId], unread: false});
    // },
}
</script>
