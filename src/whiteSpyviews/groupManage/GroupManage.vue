...
<template>
  <div id="email-app" class="border border-solid d-theme-border-grey-light rounded relative overflow-hidden">
    ...
    <vs-sidebar class="items-no-padding" parent="#email-app" :click-not-close="clickNotClose"
      ...
      <group-manage-sidebar :emailTags="emailTags" @closeSidebar="toggleEmailSidebar"
                            :mailFilter="mailFilter" ref="gmsidebar"></group-manage-sidebar>
    </vs-sidebar>
    ...
    <div :class="{'sidebar-spacer': clickNotClose}"
      ...
      <div class="flex border items-center app-search-container">
        <feather-icon class="md:inline-flex lg:hidden ml-4 mr-4 cursor-pointer" icon="MenuIcon"
                      @click.stop="toggleEmailSidebar(true)"></feather-icon>
        <vs-input icon="icon-search" size="large" icon-pack="feather" placeholder="Search Mail" v-model="searchQuery"
                  class="vs-input-no-border vs-input-no-shdow-focus w-full no-icon-border"/>
      </div>

      <!-- EMAIL ACTION BAR -->
      <div
        class="email__actions flex flex-wrap justify-between p-4 border border-r-0 border-l-0 border-solid d-theme-border-grey-light">
        <div>
          <vs-checkbox v-model="selectAllCheckBox" icon-pack="feather" :icon="selectAllIcon"
                       class="select-all-chexkbox ml-0">Select All
          </vs-checkbox>
        </div>
        <div class="flex">
          <vs-dropdown vs-custom-content vs-trigger-click class="cursor-pointer">
            ...
            <vs-dropdown-menu>
              <ul class="my-2">
                <li class="px-4 mb-2 flex items-start cursor-pointer hover:text-primary" @click="moveTo('inboxed')"
                    v-if="mailFilter != 'inbox'">
                  <feather-icon icon="MailIcon" svg-classes="h-5 w-5"/>
                  <span class="ml-3">Inbox</span>
                </li>
                <li class="px-4 mb-2 flex items-start cursor-pointer hover:text-primary" @click="moveTo('drafted')"
                    v-if="mailFilter != 'draft'">
                  <feather-icon icon="Edit2Icon" svg-classes="h-5 w-5"></feather-icon>
                  <span class="ml-3">Draft</span>
                </li>
                <li class="px-4 mb-2 flex items-start cursor-pointer hover:text-primary" @click="moveTo('spam')"
                    v-if="mailFilter != 'spam'">
                  <feather-icon icon="InfoIcon" svg-classes="h-5 w-5"></feather-icon>
                  <span class="ml-3">Spam</span>
                </li>
                <li class="px-4 flex items-start cursor-pointer hover:text-primary" @click="moveTo('trashed')"
          ...
          <vs-dropdown vs-custom-content vs-trigger-click class="cursor-pointer">
            ...
            <vs-dropdown-menu>
              <ul>
                <li v-for="(label, index) in userTags" :key="index" class="px-2 py-1 cursor-pointer"
          ...
          <feather-icon icon="MailIcon" class="ml-5 cursor-pointer" svg-classes="h-6 w-6"
                        @click="updateMarkUnread"></feather-icon>

          <feather-icon icon="TrashIcon" class="cursor-pointer ml-5" svg-classes="h-6 w-6"
                        @click="moveTo('trashed')"></feather-icon>
        </div>
      </div>
      ...
      <VuePerfectScrollbar class="email-content-scroll-area" :settings="settings" ref="mailListPS">
        <transition-group name="list-enter-up" class="email__mails" tag="ul" appear>
          <li class="cursor-pointer email__mail-item" v-for="(user, index) in userData" v-bind:key="index"
              @click.stop="showUser(user.id)"
              :style="{transitionDelay: (index * 0.1) + 's'}">
            <user-item :user="user" :isUserOpen="isUserOpen(user.id)" :isSelected="isMailSelected(user.id)"
                       @addToSelected="addToSelectedMails" @removeSelected="removeSelectedMail" @click="openSiderBar(user.id)"></user-item>
          </li>
        </transition-group>
      </VuePerfectScrollbar>
    </div>
...
<script>
  import UserItem from './GroupManageItem'
  import GroupManageView from './GroupManageView'
  import VuePerfectScrollbar from 'vue-perfect-scrollbar'
  import GroupManageSidebar from "./GroupManageSidebar";
  ...
  export default {
    ...
    watch: {
      manageGroupActive(value) {
        console.log("change", value)
        this.$vs.loading({
          type: 'sound',
        })
        getUserByGroupId(value).then(response => {
    ...
    computed: {
      manageGroupActive() {
        return this.$store.state.manageGroupActive
      },
      mailFilter() {
      ...
      isUserOpen() {
        return (userId) => userId == this.openUserId;
      },
      selectAllIcon() {
